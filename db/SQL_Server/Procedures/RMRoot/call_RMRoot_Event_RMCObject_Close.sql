...
BEGIN
       ...
       DECLARE @bError    INT,
        ...
        SELECT * INTO #CObject FROM dbo.Table_Object ()
        ...
        SELECT * INTO #PObject FROM dbo.Table_Object ()
          ...
          EXEC @bError = dbo.call_RMRoot_Event @twRMRootIx, @twEventIz OUTPUT
            ...
            IF @bError = 0
         BEGIN
                   EXEC @bError = dbo.call_RMCObject_Delete_Descendants @twRMCObjectIx_Close
           END

            IF @bError = 0
         BEGIN
                   EXEC @bError = dbo.call_RMTObject_Delete_Descendants NULL
           END
